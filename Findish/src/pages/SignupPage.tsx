...
export default function SignupPage() {
  ...
  return (
    <div className="min-h-screen bg-gradient-brand">
      ...
      <main className="flex flex-col items-center pt-16 pb-12">
        <div className="w-full max-w-150 px-4 flex flex-col gap-5">
          <h1 className="typo-h1 pt-10 font-semibold text-neutral-800">
            회원가입
          </h1>
          ...
          >
            ...
            <div className="flex flex-col gap-1">
              <Input
                ...
                rightElement={
                  <Button
                    size="sm"
                    type="button"
                    className="typo-body-sm h-9 px-3"
              ...
              {idCheckStatus === "available" && (
                <p className="typo-body-sm text-green-500">
                  사용 가능한 아이디입니다.
                </p>
              )}
              {idCheckStatus === "duplicated" && (
                <p className="typo-body-sm text-red-500">
                  이미 사용 중인 아이디입니다.
                </p>
              )}
            </div>
            ...
            {/* 이메일 */}
            <div className="flex flex-col gap-1">
              <label className="typo-body-md text-neutral-800 font-medium flex gap-0.5">
              ...
              <div className="flex items-center gap-2">
                <input
                  type="text"
                  placeholder="이메일 주소"
                  className="flex-1 h-11 px-4 rounded-[10px] border border-neutral-400 bg-white typo-body-md placeholder:text-neutral-400 outline-none focus:border-primary transition-colors"
                  {...register("emailLocal", { required: true })}
                />
                <span className="typo-body-md font-medium text-neutral-600">
                  @
                </span>
                {selectedDomain === "직접 입력" ? (
                  ...
                  <div className="flex-1 h-11 px-4 rounded-[10px] border border-neutral-400 bg-white flex items-center typo-body-md text-neutral-800">
                    {selectedDomain}
                  </div>
                )}
                <select
                  className="h-11 px-3 rounded-[10px] border border-neutral-400 bg-white typo-body-sm text-neutral-600 outline-none focus:border-primary transition-colors cursor-pointer"
            ...
            <div className="flex flex-col gap-2">
              <p className="typo-body-md font-semibold text-neutral-800">
                약관 동의
              </p>
              <div className="border border-neutral-400 rounded-[10px] overflow-hidden">
                <label className="flex items-center gap-3 px-4 py-3 cursor-pointer border-b border-[#E5E7EB]">
                  <Checkbox checked={terms.all} onChange={toggleAll} />
                  <span className="typo-body-sm font-medium text-neutral-800">
                    전체 약관 동의
                  </span>
                </label>
                ...
                {[
                  ...
                  },
                  {
                    ...
                    label: "(선택) 맞춤 마케팅을 위한 개인정보 수집 동의",
                  },
                  {
                    key: "aiPick" as const,
                    label: "(필수) AI 픽 서비스를 위한 개인정보 이용 동의",
                  },
                ].map(({ key, label }) => (
                  <label
                    key={key}
                    className="flex items-center justify-between px-4 py-2.5 cursor-pointer border-b border-[#F3F4F6] last:border-0"
                  >
                    <div className="flex items-center gap-3">
                      ...
                      />
                      <span className="typo-body-sm text-neutral-600">
                        {label}
                      </span>
                    </div>
                    <button
